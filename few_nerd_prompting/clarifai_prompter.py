...
class ClarifaiPrompter:
    ...
    def predict(self, model_id, raw_texts_ner, retries=3) -> List[str]:
        for i in range(retries):
            post_model_outputs_response = self._predict(model_id, raw_texts_ner)
            if post_model_outputs_response.status.code == status_code_pb2.SUCCESS:
                return [output.data.text.raw for output in post_model_outputs_response.outputs]
            if i == retries - 1:
